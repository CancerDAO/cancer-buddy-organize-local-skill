...
If `mode: curated_text`, stop before the full workflow and execute
[`curated-text-mode.md`](curated-text-mode.md) as the authoritative workflow. Never wrap the text in a file to re-enter the full pipeline.
...
- `mode` (`full` | `merge_only` | `v1_upgrade` | `curated_text`)
- `curated_text` (required only for `curated_text`; owner-reviewed UTF-8 text)
