...
│                            # scheme_version 3 — 14 clinical domains (+2 infra); authoritative def: skills/cancer-buddy-organize-local/references/bucket-taxonomy.md §1
├── 01_身份与基础信息/ 02_既往史与家族史/ 03_病程与叙事文书/ 04_诊断与分期/
├── 05_影像/ 06_分子与组学/ 07_检验/ 08_治疗/ 09_手术与操作/
├── 10_随访与监测/ 11_会诊与转诊/ 12_心理社会与支持/ 13_行政与财务/ 14_患者自管补充/
├── 90_原始文件镜像/ 99_无关文件/   # HIDDEN infra (never patient-visible, never anchored)
├── longitudinal_observations.json  # longitudinal stream store (timeseries / trended structured)
  ...
  "review_flags": [
    {
      ...
      "source_evidence": ["04_诊断与分期/诊断证明/出院诊断证明_2024-07-05.jpg"],
...
Each entry MUST contain: `id` (`RF-NNN`), `severity` ∈ {red, yellow, green}, `category` (from table), `field_path` (dot path into profile.json), `current_value`, `issue` (≤80 chars Chinese summary), `source_evidence[]` (paths to OCR sidecars or filed clinical-domain bucket entries that support `current_value`), `suggested_action`, `user_confirmed: false` on first write.
